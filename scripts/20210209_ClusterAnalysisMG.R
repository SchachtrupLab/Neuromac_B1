...
#' Plot some myeloid-associated genes.
#+ myeloid_marker_vln, fig.height=6, fig.width=11 
myeloid_genes <- c('P2ry12','Tmem119','Cx3cr1','Csf1r','Adgre1','Ptprc','Ly6c2',
                   'Ccr2','Spp1','Apoe','Trem2','Tyrobp','Ms4a7','Lyve1','Mrc1',
                   'Ldha','Cd68','Cd63','Cd63','Cd83','Lamp1','C1qa','Mki67',
                   'Top2a')
mg$seurat_clusters <- mg$SCT_snn_res.0.4
...
ggsave(
  filename = paste0(results_out, 'MG_subclusters_myeloid-genes_vln.svg'),
  plot = myeloid_vln,
  device = 'svg',
  height = 6, width = 11
)
...
#' Build UMAPs plotting gene expression.  
Idents(mg) <- 'seurat_clusters'
DefaultAssay(mg) <- 'RNA'
...
ggsave(filename = paste0(results_out, 'MG_myeloid-genes_umap.svg'),
       plot = myeloid_umap, device = 'svg', height = 12, width = 14.5)
myeloid_umap
