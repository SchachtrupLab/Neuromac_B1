...
results_outpath <- '../results/20200922_PreliminaryClustering/'
set.seed(123)
```
...
dat_sce <- scater::addPerCellQC(dat_sce)
dat_sce <- scater::addPerFeatureQC(dat_sce)
dat_sce[['log10_total_counts']] <- log10(x = dat_sce$total)
dat_sce[['log10_total_features']] <- log10(x = dat_sce$detected)
qc_metrics <- c('log10_total_counts',
                ...
                'percent_top_500',
                'percent.mt')
tmp <- as.data.frame(colData(dat_sce)[qc_metrics])
...
qc_threshold <- apply(X = dat@meta.data[qc_metrics],
                      MARGIN = 2,
                      FUN = function(x) {
...
qc_umap <- vector(mode = 'list', length = length(qc_metrics))
names(qc_umap) <- qc_metrics
tmp <- cbind(dat@meta.data, dat[['umap']]@cell.embeddings)
for(metric in qc_metrics) {
  qc_umap[[metric]] <- tmp %>%
    ggplot(mapping = aes_string(x = 'UMAP_1', y = 'UMAP_2')) +
...
qc_umap <- cowplot::plot_grid(plotlist = qc_umap, ncol = round(sqrt(length(qc_metrics))))
qc_umap
ggsave(filename = paste0(results_outpath, 'preliminary_clustering_QC_umap_adaptiveThresholds.tiff'), plot = qc_umap, device = 'tiff', height = 9, width = 10.75)
```
...
```{r qc_umap2, fig.height=6, fig.width=7}
qc_metric <- c('log10_total_features',
               'log10_total_counts',
               'percent_top_200',
               'percent.mt')
qc_umap <- vector(mode = 'list', length = length(qc_metric))
names(qc_umap) <- qc_metric
tmp <- cbind(dat@meta.data, dat[['umap']]@cell.embeddings)
...
qc_umap <- cowplot::plot_grid(plotlist = qc_umap, ncol = round(sqrt(length(qc_metric))))
qc_umap
ggsave(filename = paste0(results_outpath, 'preliminary_clustering_QC_umap_flatThresholds.tiff'), plot = qc_umap, device = 'tiff', height = 6, width = 7)
```
...
dat <- RunUMAP(object = dat, dims = npcs)
```

```{r graph_clustering_UMAP, fig.width=11, fig.height=4.5}
plot1 <- DimPlot(dat, group.by = 'orig.ident', reduction = 'umap', pt.size = 2) + 
  theme(axis.line = element_line(size = 0.5),
        axis.ticks = element_blank(), 
        axis.text = element_blank(), 
        axis.title = element_text(size = 10), 
        panel.border = element_rect(color = 'black', fill = NA))
plot2 <- DimPlot(dat, group.by = 'seurat_clusters', label = TRUE, label.size = 6, reduction = 'umap', pt.size = 2) + 
  theme(axis.line = element_line(size = 0.5),
        axis.ticks = element_blank(), 
        axis.text = element_blank(),
        axis.title = element_text(size = 10), 
...
plot <- cowplot::plot_grid(plot1, plot2, ncol = 2, rel_widths = c(1,0.925))
plot
ggsave(filename = paste0(results_outpath, 'SVZ_UMAP.tiff'), plot = plot, device = 'tiff', height = 4.5, width = 11)
```
...
```{r myeloid_gene_UMAP, fig.align='center', fig.width=14, fig.height=10}
...
extended_plots
ggsave(filename = paste0(results_outpath, 'myeloid_genes_UMAP.tiff'), plot = extended_plots, device = 'tiff', height = 10, width = 14)
```


Some genes related to quiescent NSCs (qNSCs).  

```{r qNSC_gene_UMAP, fig.align='center', fig.width=16.8, fig.height=12.5}
...
extended_plots
ggsave(filename = paste0(results_outpath, 'qNSC_genes_UMAP.tiff'), plot = extended_plots, device = 'tiff', height = 12.5, width = 16.8)
```


Some genes related to primed NSCs.  

```{r primedNSC_gene_UMAP, fig.align='center', fig.width=14, fig.height=7.5}
...
extended_plots <- cowplot::plot_grid(plotlist = extended_plots, ncol = 5)
extended_plots
ggsave(filename = paste0(results_outpath, 'primedNSC_genes_UMAP.tiff'), plot = extended_plots, device = 'tiff', height = 7.5, width = 14)
```


Some genes related to activated NSCs (aNSCs).  

```{r aNSC_gene_UMAP, fig.align='center', fig.width=16.8, fig.height=17.5}
...
extended_plots <- cowplot::plot_grid(plotlist = extended_plots, ncol = 6)
extended_plots
ggsave(filename = paste0(results_outpath, 'aNSC_genes_UMAP.tiff'), plot = extended_plots, device = 'tiff', height = 17.5, width = 16.8)
```
...
```{r qNSC_gene_vln, fig.align='center', fig.width=12, fig.height=17.5}
some_markers <- c('Aldoc','Aqp4','Atp1a2','Bcan','Cldn10','Clu','Fgfr3','Gja1','Gpr37l1','Htra1','Prdx6','Prnp','Ptn','Slc1a2','Slc1a3','Slc6a1','Sparcl1','Tspan7','Id2','Id3','Id4','Sox9','Aldh1l1','Fabp7','Fasn','Hes5','Nr2e1','Notch2')
DefaultAssay(dat) <- 'RNA'
Idents(dat) <- 'seurat_clusters'
extended_plots <- VlnPlot(object = dat, features = some_markers, slot = 'data', combine = FALSE, pt.size = 0)
...
```{r primedNSC_gene_vln, fig.align='center', fig.width=12, fig.height=7.5}
some_markers <- c('Cpe','Cst3','Dbi','Dkk3','Fxyd1','Gstm1','Igfbp5','Itm2b','Mt1','Ntrk2','Prdx6','S100a6','Thbs4')
DefaultAssay(dat) <- 'RNA'
Idents(dat) <- 'seurat_clusters'
extended_plots <- VlnPlot(object = dat, features = some_markers, slot = 'data', combine = FALSE, pt.size = 0)
...
```{r aNSC_gene_vln, fig.align='center', fig.width=12, fig.height=22.275}
some_markers <- c('Acot1', 'Ascl1', 'Ckb', 'Eef1a1', 'Egfr', 'Lima1', 'Luc7l3', 'Miat', 'Mycn', 'Npm1', 'Olig2', 'Pabpc1', 'Ptma', 'Rpl12', 'Rpl41', 'Rpsa', 'Dll1', 'Dll3', 'Fos', 'Ccnd1', 'E2f1', 'Foxm1', 'Myc', 'Cdk2', 'Ldhb', 'Aurkb', 'Dcx', 'Gad1', 'Mki67', 'Sp8', 'Jag1', 'Ntsr2', 'Rpl32', 'Ccna2', 'Ccnd2', 'Pax6', 'Cdk1', 'Dlx1', 'Dlx2')
DefaultAssay(dat) <- 'RNA'
Idents(dat) <- 'seurat_clusters'
extended_plots <- VlnPlot(object = dat, features = some_markers, slot = 'data', combine = FALSE, pt.size = 0)
...
```{r filter_LR, eval=FALSE, echo=TRUE, results='hide', warning=FALSE, message=FALSE}
...
```{r calculate_LR, eval=FALSE, echo=TRUE, results='hide', warning=FALSE, message=FALSE}
...
saveRDS(dat, file = paste0(results_outpath, '20200922_SVZ.rds'))
sessionInfo()
```