...
results_outpath <- '../results/20200701_subclustering_microglia/'
data_outpath <- '../data/20200625_microglia.rds'
set.seed(123)
```
...
npcs <- 1:10 # This was used for results sent to Suvra
...
saveRDS(object = microglia, file = '../data/20200625_microglia.rds')
```
...
```{r GeneOntology, results='hide', fig.width=20, fig.height=14}
library('biomaRt')
library('topGO')
library('org.Mm.eg.db')

microglia <- readRDS(file = data_outpath)
...
markers <- readRDS(file = paste0(results_outpath, 'microglia_de_markers.rds'))
conversion_input <- rownames(microglia[['RNA']]@counts)
...
ensembl_conversion <- getBM(attributes = c('mgi_symbol','ensembl_gene_id','external_gene_name'),
                            ...
                            mart = ensembl)
# table(ensembl_conversion$external_gene_name %in% rownames(microglia[['RNA']]@counts))
...
global_set <- plyr::mapvalues(x = global_set,
                              ...
                              warn_missing = FALSE)
markers$gene <- plyr::mapvalues(x = markers$gene,
                                from = c('Sepp1'), 
                                to = c('Selenop'))
markers$ensembl <- plyr::mapvalues(x = markers$gene, 
                                   from = ensembl_conversion$external_gene_name,
                                   to = ensembl_conversion$ensembl_gene_id,
                                   warn_missing = FALSE)
# markers$ensembl <- plyr::mapvalues(x = markers$ensembl,
#                                    from = ensembl_conversion$external_gene_name,
#                                    to = ensembl_conversion$ensembl_gene_id,
#                                    warn_missing = FALSE)
no_match <- unique(markers$gene[markers$gene %in% global_set[!grepl('ENS', global_set)]]) # Atp5o.1
markers <- markers[!markers$gene %in% no_match,]

markers$cluster <- as.numeric(as.character(markers$cluster))

subtype <- levels(microglia$seurat_clusters)
GO_results <- vector(mode = 'list')
for(i in 1:length(subtype)) {
  # upregulated
  tmp <- markers[markers$avg_logFC > 0 & markers$cluster == (i-1),] %>% .[c('gene','p_val_adj','ensembl')]
...
for(i in 1:length(subtype)) {
  ...
  GO_plots[[i]] <- tmp[1:20,] %>%
    ...
    ylab("Enrichment") +
    labs(title = paste('Cluster', i, 'GO Enrichment')) +
