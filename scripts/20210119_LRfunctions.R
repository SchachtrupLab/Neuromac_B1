...
calculateLR <- function(
  ...
  for(i in 1:nrow(var_set)) {
    ...
    tmp_nulls <- 1/2 * (null_avg_lig + null_avg_rec)
    colnames(tmp_nulls) <- paste(colnames(null_avg_lig), colnames(null_avg_rec), sep = '_')
    
    # For max-T method, take element-wise maximum values (this iterates nrow(var_set) times)
    # null_scores[[i]] <- tmp_nulls
    if (adjust_pval) {
      maxT_scores[tmp_nulls > maxT_scores] <- tmp_nulls[tmp_nulls > maxT_scores]
    }
...
plotLR <- function(
  ...
  # Remove rows where p-value is greater than max_pval, if provided.
  if (!is.null(max_pval)) {
    tmp_results <- tmp_results %>% filter(pval < max_pval)
  }
